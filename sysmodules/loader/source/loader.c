...
static Result LoadProcessImpl(Handle *outProcessHandle, const ExHeader_Info *exhi, u64 programHandle)
{
    ...
    if (R_SUCCEEDED(res = loadCode(exhi, programHandle, &mapped)))
    {
        ...
        res = svcCreateCodeSet(&codeset, &csh, mapped.text_addr, mapped.ro_addr, mapped.data_addr); // PLG_MARKER(coin_loader_createcodeset_call)
        if (R_SUCCEEDED(res))
        {
            // There are always 28 descriptors
            res = svcCreateProcess(outProcessHandle, codeset, exhi->aci.kernel_caps.descriptors, count); // PLG_MARKER(coin_loader_createprocess_call)
