...
void menuThreadMain(void)
{
    ...
    while(!preTerminationRequested)
    {
        ...
        u32 kHeld = scanHeldKeys(); // PLG_MARKER(powerprevent_marker_key_scan)
...
void menuEnter(void)
{
    ...
    if(!menuShouldExit && menuRefCount == 0)
    {
        ...
        if (R_FAILED(Draw_AllocateFramebufferCache(FB_BOTTOM_SIZE)))
        {
            ...
            Draw_SetupFramebuffer(); // PLG_MARKER(blur_marker_menu_entered)
    }
    Draw_Unlock();
}
...
void menuLeave(void)
{
    ...
    if(--menuRefCount == 0)
    {
        Draw_RestoreFramebuffer(); // PLG_MARKER(blur_marker_menu_leaving)
...
static void menuDraw(Menu *menu, u32 selected)
{
    ...
    if (isRelease) { // PLG_MARKER(blur_marker_menudraw_start)
    ...
    Draw_FlushFramebuffer(); // PLG_MARKER(blur_marker_menudraw_end)
}
