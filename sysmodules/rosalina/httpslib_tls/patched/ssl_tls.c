...
#if defined(BLUR_TLS_MIN_CERT)
static int blur_tls_skip_tag(unsigned char **p,
...
static int blur_tls_find_spki(unsigned char *crt,
                              size_t crt_len,
                              unsigned char **spki,
                              size_t *spki_len)
{
    ...
    if (p < tbs_end && *p == (MBEDTLS_ASN1_CONTEXT_SPECIFIC |
        ...
        if (blur_tls_skip_tag(&p, tbs_end,
    ...
    if (blur_tls_skip_tag(&p, tbs_end, MBEDTLS_ASN1_INTEGER) != 0 ||
        blur_tls_skip_tag(&p, tbs_end,
...
int mbedtls_ssl_parse_certificate(mbedtls_ssl_context *ssl)
{
    ...
    ret = blur_tls_find_spki(ssl->in_msg + i, crt_len, &spki, &spki_len);
    if (ret != 0) {
        goto exit;
    }
